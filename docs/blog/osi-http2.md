...
最近读完《HTTP2 基础教程》，相当于是对 HTTP2 有了个大致的了解。

## HTTP 发展史

随着时代的不断发展，人们对于网站的需求越来越高，最开始的网站仅仅是展示文字信息，到现在，一个网站还需要展示图片，多媒体，特效等。
...
HTTP1.1 已经存在 20 年了，却还是当前社会中使用最广泛的协议，同时也暴露出一些问题：
...
使用 Expires 首部，将资源`失效的日期`告诉客户端，在失效如期之前，客户端都会直接使用缓存中的资源而不会发起请求。
...
在请求实体中包含实体校验码 ETag，它唯一标识所请求的资源，服务器会比较当前 ETag 于请求首部中收到 ETag，如果一致，就返回 304.
...
HTTP2 其实并没有要求必须使用 HTTPS，只是人们正好认识到 HTTPS 的重要性，而 HTTP2 正好出来，就一起成为了大家的优化手段。
...
而 HTTP2 比 HTTP1 更容易丢包，是因为 h1 会开启 6 个连接，默认的拥塞窗口大。
...
HTTP 是使用 TCP 作为传输层的协议，由于 TCP 是可靠的，拥塞控制的协议，在进行一次连接时，会发生三次握手，断开连接时会有 4 次挥手，所以有人提出可以用 udp 这种简单，快速的协议去代替。

Google 开发的 QUIC 是极快的 UDP 网络连接，提供了 HTTP2 等效的，多路复用，流量控制，TSL 等效的安全机制，以及 TCP 等效的连接语义、可靠性、拥塞机制。

TLS（传输层安全协议，用来实现 HTTPS）也在不断地进行改进。在 TLS1.3 版本中 新连接只需要一次往返（目前最少是三次），如果是恢复连接，则不需要往返延时。
