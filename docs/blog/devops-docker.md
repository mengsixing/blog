...
最近在学习 Docker，并用 Docker 重新部署了[cdfang-spider](https://github.com/mengsixing/cdfang-spider)项目，使用 docker 后确实大幅度地降低了部署难度。如果你也想用 Docker 来部署自己的项目，那就让我们一起往下看。
...
```shell
docker pull mengsixing/cdfang-spider
docker pull mongo
docker network create webapp-network
docker run -d --network webapp-network -v ~/data/db:/data/db mongo
docker run -p 8082:8082 --network webapp-network -d mengsixing/cdfang-spider
```
...
services:
  database:
    ...
    networks:
      - webapp-network
  web:
    image: mengsixing/cdfang-spider
    depends_on:
      - database
    ports:
      - 8082:8082
...
接下来以 [cdfang-spider](https://github.com/mengsixing/cdfang-spider) 项目为例，使用 Docker 部署项目。
...
```shell
docker build -t mengsixing/cdfang-spider .
```
