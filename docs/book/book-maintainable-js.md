...
- 禁止使用 eval，Function，setTimeOut，setInterval 的字符串形式。
- 三元运算符仅仅用在赋值语句上，不要成为 if else 的替代品。

:::tip 提示
string 类型可以调用 String 包装器的方法，是因为语句的背后，js 引擎创建了 string 类型的新势力，紧跟着就销毁了，所以给 string 类型上添加属性和方法是没有用的。
:::
...
- **单全局变量**
  ...
  - 引用命名空间概念：
    - 调用 namespace() 来申明开发者将要使用的命名空间，即：将对象挂载到哪个对象下，如果不存在，就创建。
- **AMD 模块化**
...
- **零全局变量**
  - 如果你的脚本非常短，并且不需要和其他代码产生交互，可以使用一个**自执行函数来实现模块化**。
...
JS 检测自定义类型时，最好的做法是使用 instanceof，这也是唯一的方法。
...
instanceof 还可以检测引用对象的值。但可能会检测到原型对象上。
...
使用 Object.prototype.toString.call(value) === '[object Array]' 检测数组效果很不错，在 ES6 中可以通过 的 Array.isArray 来实现。
...
我们在开发项目时，遇到错误其实并不可怕，可怕的将这些错误带到线上环境中去，为此，在程序的一些关键地方，使用自定义错误，可以让我们在上线前提前发现问题，避免错误影响变大。编写自定义错误可以遵守一下几点：
...
在我们编写代码的时候，会用到很多其他对象，一些是存在于上下文作用域中的、一些是存在于其他库里的等等，这些对象都不是我们自己定义的。对于这些对象，我们要做到完全不去改动。
...
为特定浏览器的特性进行测试，并仅当特性存在时即可应用特性检测。
